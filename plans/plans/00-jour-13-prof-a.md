...
**Livré :** `GET /me` enrichi **sans** `role` ; `PATCH /me` (nom / username / phone) ; `GET /users/{id}` + privacy ; avatar SKIPPED lab ; `org_resolver`. 0 migration.
...
| **Ma fiche, photo, fiche collègue**     | **PROF-A**   | **fait** (ce jour)              |
...
**Livré (code) :** `display_name` / `avatar_url` / `org` / `email_pending` / `last_seen` / `region` objet sur `GET /me` ; `PATCH /me` ; `GET /users/{id}` ; `POST/DELETE /me/avatar` + `GET /media/files/{id}`. Login `public_user` **inchangé**.
...
- [x] GET `/me` enrichi **sans** `role` ; login `public_user` inchangé
- [x] PATCH `/me` (nom / username / phone) + `FIELD_FORBIDDEN` / `USERNAME_TAKEN`
- [x] GET `/users/{id}` + privacy + 404 pending/inactif + `USE_ME`
- [x] `org_resolver` (path, manager, region IAM distincte)
- [x] POST/DELETE avatar SKIPPED lab + GET fichier
- [x] `test_prof_a.py` + régression F/R
- [x] SIRH non modifié
