...
**Préalable :** jour 0 + jour 1 (AUTH-A) + jour 2 (admin + Swagger) — **clos**.  
...
| ----------------------------------------------- | ---------- | ----------------------------- |
| Connexion identifiant + mot de passe            | AUTH-A     | **fait** (jour 1, clos)       |
| **Inscription AD / hors AD**                    | **AUTH-D** | **fait** (ce jour)            |
| **Connexion compte Windows / Active Directory** | **AUTH-B** | **fait** (ce jour)            |
| **Scanner le QR Google Authenticator**          | **AUTH-C** | **fait** (jour 4) — [00-jour-4-auth-c.md](00-jour-4-auth-c.md) |
| **Valider le code Authenticator (6 chiffres)**  | **AUTH-C** | **fait** (jour 4) |
...
| D05 | JWT + rôle `ADMIN` (`is_staff` déjà AUTH-A/jour 2). `GET /admin/users` = **tous** les users ; `?pending=true` = file RH. Approve **et** reject (`reason` obligatoire, audit seulement) | AUTH-R : `HasPermission` (`iam.user.approve` / `reject` / `read`) |
...
| `GET /api/v1/admin/users`                            | Tous les users (JWT + ADMIN)                                                                                                                  |
| `GET /api/v1/admin/users?pending=true`               | File RH (`pending_approval=true`)                                                                                                             |
| `POST /api/v1/admin/users/{id}/approve`          | `is_active=true`, `pending_approval=false`. **400** si `ldap_dn`                                                                                      |
| `POST /api/v1/admin/users/{id}/reject`           | Reste inactif + motif **audit** (`reason` obligatoire). **400** si `ldap_dn`                                                                   |
...
| D05 `GET /admin/users`   | tous les users ; `?pending=true` = file RH                             |
| D05 approve              | `is_active=true` ; login MDP 200                                       |
| D05 reject               | reste inactif ; audit `reason` ; login toujours 403                    |
| D05 reject si `ldap_dn`  | 400 `LDAP_MANAGED`                                                     |
...
- [x] App `config` + tables `system_settings` / `scheduled_jobs`
- [x] `ldap_service` **avant** les vues register / login LDAP
- [x] Seed **référentiels** (régions + `YAS`) ; **0** user AD en seed ; jean.dupont `ldap_dn` NULL
- [x] `POST /register/check-ad` + `POST /register/ad` + `POST /register` dans Swagger
- [x] D02 = seul JIT ; `login/ldap` ne crée jamais de user
- [x] `POST /login/ldap` testé sur un compte **D02**, pas sur seed
- [x] 401 unique login LDAP (inconnu YAS / bind KO / AD non loggable / rate-limit)
- [x] 403 YAS seulement **après** bind AD OK (`ACCOUNT_PENDING` / `DISABLED` / `LOCKED`)
- [x] 503 si annuaire injoignable ; **pas** de fallback automatique vers AUTH-A
- [x] `complete_login` réutilisé (D02 `PASSWORD`, login LDAP `LDAP`)
- [x] Job AUTH-16 coupe `is_active` (users avec `ldap_dn` seulement)
- [x] D05 approve/reject (rôle ADMIN) ; pending → 403
- [x] 0 secret LDAP dans `.env` / git
- [x] AUTH-A, `/admin/`, `/health` intacts
- [x] SIRH non modifié
...
Jour 4 : [00-jour-4-auth-c.md](00-jour-4-auth-c.md) — **AUTH-C**. Plus de JWT tant que `POST /mfa/verify` n’est pas OK.